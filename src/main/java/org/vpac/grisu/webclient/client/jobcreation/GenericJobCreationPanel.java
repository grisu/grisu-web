...
public class GenericJobCreationPanel extends LayoutContainer implements JobSubmissionFinishedEvent.Handler, ValueChangeHandler<List<GrisuFileObject>> {
	...
	private ListStore<GrisuFileObject> inputFileStore = new ListStore<GrisuFileObject>();
	...
	public GenericJobCreationPanel() {
		...
		// load last values
		GrisuClientService.Util.getInstance().getUserProperty(
				Constants.GENERIC_JOB_LAST_CPUS, new AsyncCallback<String>() {
					...
					public void onSuccess(String arg0) {
						...
						if (arg0 != null && !"".equals(arg0)) {
							try {
								Integer value = Integer.parseInt(arg0);
								getCpusComboBox().setSimpleValue(value);
							} catch (Exception e) {
								e.printStackTrace();
							}
						}
		...
		GrisuClientService.Util.getInstance().getUserProperty(
				Constants.GENERIC_JOB_LAST_EMAIL, new AsyncCallback<String>() {
					...
					public void onSuccess(String arg0) {
						...
						if (arg0 != null && !"".equals(arg0)) {
							getEmailTextField().setValue(arg0);
						}
		...
		GrisuClientService.Util.getInstance().getUserProperty(
				Constants.GENERIC_JOB_LAST_USE_EMAIL,
				new AsyncCallback<String>() {
					...
					public void onSuccess(String arg0) {
						...
						if (arg0 != null && arg0.toLowerCase().equals("true")) {
							getCheckBox().setValue(Boolean.TRUE);
						}
					}
				});
		...
		GrisuClientService.Util.getInstance().getUserProperty(
				...
				new AsyncCallback<String>() {
					...
					public void onSuccess(String arg0) {
						...
						try {
							walltimeInMinutes = Integer.parseInt(arg0);
						} catch (Exception e) {
							return;
						}
		...
		fileSelectorWindow = new FileSelectorAndUploadWindow(Constants.GENERIC_JOB_LAST_INPUTFILE_URL);
		...
		dragSource = new DragSource(getListView()) {
			...
			protected void onDragStart(DNDEvent e) {  
				...
				List<GrisuFileObject> selectedFiles = getListView().getSelection();
			    ...
			    if (selectedFiles.size() > 0) {
			      ...
			      if (getStatusText() == null) {
			        e.getStatus().update(GXT.MESSAGES.grid_ddText(selectedFiles.size()));
			      } else {
			        e.getStatus().update(Format.substitute(getStatusText(), selectedFiles.size()));
			      }
			    }
		...
		dropTarget = new DropTarget(getCommandLineTextArea()) {
			...
			protected void onDragDrop(DNDEvent event) {  
			    ...
			    String firstPart = currentValue.substring(0, insertPoint).trim();
			    String lastPart;
				...
				GrisuClientService.Util.getInstance().getFqans(
						new AsyncCallback<String[]>() {
							...
							public void onSuccess(String[] arg0) {
								...
								voComboBox.setRawValue(firstValue);
	...
	private Button getRemoveFileButton() {
		...
		if ( removeFileButton == null ) {
			...
			removeFileButton.addListener(Events.Select, new Listener<BaseEvent>() {
	...
	private TextArea getCommandLineTextArea() {
		if (txtrEnterTheCommand == null) {
			...
			txtrEnterTheCommand
					.setFieldLabel("Enter the command to run");
				...
				apps, fqan, new AsyncCallback<String[]>() {
					...
					public void onSuccess(final String[] versions) {
						...
						if (currentVersion == null || "".equals(currentVersion)) {
							...
							// TODO maybe get last used value?
						...
						// // trying to get last default version
						// GrisuClientService.Util.getInstance().getUserProperty(
						// Constants.LAST_VERSION + "_" + app,
						// new AsyncCallback<String>() {
						//
						// public void onFailure(Throwable arg0) {
						//
						...
						// + " versions for "
						// + app
						// + ". Didn't find last used version, using default.");
						// } else {
						// setStatus("Found "
						// + versions.length
						// + " versions for "
						// + app
						// + ". Found last used version: "
						// + arg0);
	...
	private void submitJob() {
		...
		final Timer t = new Timer() {
			...
			public void run() {
				...
				GrisuClientService.Util.getInstance().getCurrentStatus(jobProperties.get(Constants.JOBNAME_KEY), new AsyncCallback<DtoActionStatus>() {
					...
					public void onSuccess(DtoActionStatus arg0) {
						...
						try {
							...
							bar.updateProgress(current/total, arg0.getLog().get(arg0.getLog().size()-1).getLogMessage());
